...
func TestRenderFooterNarrowKeysTruthful(t *testing.T) {
	...
	got := renderFooter(s, l, [3]int8{0, 0, 0}, DefaultSections(), 60, false)
...
func TestRenderFooterEffectiveStateMatchesVisibility(t *testing.T) {
	...
	got := renderFooter(s, l, [3]int8{0, 0, 0}, DefaultSections(), 100, false)
	...
	got = renderFooter(s, l, [3]int8{0, 0, 0}, DefaultSections(), 0, false)
	...
	got = renderFooter(s, l, [3]int8{1, -1, 0}, DefaultSections(), 0, false)
...
func TestRenderFooterShowsMaxPerformanceState(t *testing.T) {
	s := NewStyles(true)
	l := Layout{Width: 100}
	got := renderFooter(s, l, [3]int8{}, DefaultSections(), 40, true)
	if !strings.Contains(got, "[p]max-perf:on") {
		t.Fatalf("footer missing active max-performance state: %q", got)
	}
}
